...
class WelcomeWindow(Gtk.Window):
    ...
    def __init__(self):
        ...
        # Crear y cargar el estado del botón de autoinicio
        self.startup_check = Gtk.CheckButton(label="No iniciar al encender")
        main_box.pack_end(self.startup_check, False, False, 0)
        self.load_startup_check_state()  # Cargar el estado guardado del botón de autoinicio
    ...
    def on_exit_button_clicked(self, button):
        self.save_startup_check_state()  # Guardar el estado del botón de autoinicio antes de salir
        Gtk.main_quit()
    ...
    def save_startup_check_state(self):
        # Guardar el estado del botón de autoinicio en un archivo de configuración
        config = configparser.ConfigParser()
        config['Startup'] = {'No iniciar al encender': str(self.startup_check.get_active())}
        with open('config.ini', 'w') as configfile:
            config.write(configfile)

    def load_startup_check_state(self):
        # Cargar el estado del botón de autoinicio desde el archivo de configuración
        config = configparser.ConfigParser()
        if os.path.exists('config.ini'):
            config.read('config.ini')
            startup_state = config['Startup'].get('No iniciar al encender', 'False')
            self.startup_check.set_active(startup_state.lower() == 'true')
